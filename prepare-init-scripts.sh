...
if [ ${#db1[@]} -ge 5 ]; then
    while read -r db_name db_tables db_username db_host db_password db_privileges; do
        ...
        sed -i s/REPLACE_DATABASE_TABLE/"${db_tables}"/g ./mariadb/init/1.sql
...
#         sed -i s/REPLACE_DATABASE_TABLE/"${db_tables}"/g ./mariadb/init/2.sql
...
#         sed -i s/REPLACE_DATABASE_TABLE/"${db_tables}"/g ./mariadb/init/3.sql
...
#         sed -i s/REPLACE_DATABASE_TABLE/"${db_tables}"/g ./mariadb/init/4.sql
...
#         sed -i s/REPLACE_DATABASE_TABLE/"${db_tables}"/g ./mariadb/init/5.sql
